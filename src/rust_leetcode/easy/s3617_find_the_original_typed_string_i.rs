...
//! typing, she is aware that she may still have done this at most once.
//! You are given a string word, which represents the final output displayed on
//! Alice's screen. Return the total number of possible original strings that
...
mod tests {
    ...
    #[test]
    fn test_3617() {
        assert_eq!(Solution::possible_string_count("aabbccc"), 5);
        assert_eq!(Solution::possible_string_count("abcd"), 1);
        assert_eq!(Solution::possible_string_count("aaaa"), 4);
        assert_eq!(Solution::possible_string_count("a"), 1);
    }
}
